...
class BuscadorController extends Controller
{
    public function index() {
        ...
        foreach ($telefonosDuplicados as $telefono) {
            ...
            Notas::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
            Paquetes::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
            NotasPedidos::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
            Reporte::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
            Alertas::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
            ConsentimeintoJacuzzi::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
            ConsentimientoCorporal::where('id_client', $clientePrincipal->id)->update(['id_client' => $clientePrincipal->id]);
